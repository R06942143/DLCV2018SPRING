...
def get_data_label(path,label_path,folder_path):
    for i,folder in enumerate(folder_path):
        train_path = os.path.join(path,folder)
        data = sorted(os.listdir(train_path))
        frames = []
        for i in range(len(data)):
            frame = io.imread(os.path.join(train_path,data[i]))
            frames.append(frame)
        label = os.path.join(label_path,folder+'.txt')
        
    return np.array(frames).astype(np.uint8),np.genfromtxt(label,dtype = 'int')


train_X,train_Y = get_data_label(train_path,train_label_path,train_folder)
valid_X,valid_Y = get_data_label(valid_path,valid_label_path,valid_folder)
...
videoRNN.fit(feature_X, train_Y, epochs=50, batch_size=64,validation_data=(feature_V,valid_Y),callbacks=[checkpoint],verbose=1)
